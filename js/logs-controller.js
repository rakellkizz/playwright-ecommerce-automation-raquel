  ...
  function adicionarLog(cenario, log) {
    ...
    // ======================================================================
  // 🧠 IA — Registro de perguntas investigativas como log técnico
  // ----------------------------------------------------------------------
  // ✔ Usa o MESMO localStorage
  // ✔ Aparece na timeline
  // ✔ Entra nos relatórios (PDF / Allure)
  // ✔ Não interfere em logs humanos
  // ======================================================================
  function registrarPerguntaIA(cenarioId, pergunta) {
    if (!cenarioId || !pergunta) return;

    const log = {
      tipo: "ia-pergunta",
      timestamp: Date.now(),
      tecnico: "IA Investigativa",
      acao: "Pergunta investigativa",
      justificativa: pergunta,
    };

    adicionarLog(cenarioId, log);
  }
  ...
  // ======================================================================
  // 18.8 DISPARO DE ANÁLISE TÉCNICA (BASEADO NO LOCALSTORAGE)
  // ----------------------------------------------------------------------
  // ⚠ IMPORTANTE:
  // - Este sistema NÃO usa um array global de eventos
  // - Os eventos vivem organizados por cenário no localStorage
  // - Esta função APENAS lê e interpreta esses dados
  // - NÃO altera logs, NÃO grava nada, NÃO muda fluxo existente
  // ======================================================================

  function executarAnaliseTecnica() {
    try {
      // --------------------------------------------------------------
      // 1. Carrega TODOS os logs salvos no sistema
      // Estrutura retornada:
      // {
      //   login:   [ {...}, {...} ],
      //   checkout:[ {...} ],
      ...
      // }
      // --------------------------------------------------------------
      const todosOsLogs = carregarLogs();

      // --------------------------------------------------------------
      // 2. Normaliza os logs em uma lista única de eventos
      // Cada evento recebe o cenarioId para contexto
      // --------------------------------------------------------------
      const eventosNormalizados = [];

      Object.entries(todosOsLogs).forEach(([cenarioId, logs]) => {
        logs.forEach((log) => {
          eventosNormalizados.push({
            ...log,
            cenarioId, // contexto essencial para análise
          });
        });
      });

      // --------------------------------------------------------------
      // 3. Se não houver eventos, não há o que analisar
      // --------------------------------------------------------------
      ...
      // --------------------------------------------------------------
      // 4. Envia os eventos para o módulo analyzer
      // O analyzer é PURAMENTE interpretativo
      // --------------------------------------------------------------
      const resultado = analisarEventos(eventosNormalizados);

      // ======================================================
      // 🔔 DISPARO DO EVENTO GLOBAL PARA A IA INVESTIGATIVA
      // - PRECISA ter: id + resultado
     // - timestamp é opcional, pode manter
    // ======================================================
      window.dispatchEvent(
      new CustomEvent("ia:analyzer_result", {
      detail: {
      id: cenarioId,          // ✅ ID do cenário (obrigatório)
      resultado,              // ✅ resultado do analyzer (obrigatório)
      timestamp: Date.now(),  // opcional (ok manter)
      },
    })
  );
      // ==============================================================
      // 5. Log técnico (POC)
      // Futuramente: cards, HUD SOC, histórico, IA
      // ==============================================================
      console.group("📊 Análise Técnica Consolidada");
      console.log(resultado);
      console.groupEnd();

      return resultado;
    } catch (erro) {
      console.error("Erro ao executar análise técnica:", erro);
      return null;
    }
  }
